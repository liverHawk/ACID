...
class AdaptiveClusteringOptimized(nn.Module):
    """
    最適化版AdaptiveClustering - 元のEncoderを使用（安定性を優先）
    元のAdaptiveClusteringと同じ動作を保証しつつ、トレーニングループを最適化
    """
    def __init__(self, encoder_dims=(60, 40, 20), kernel_size=3, n_kernels=None, subnet_dims=[50, 30]):
        super(AdaptiveClusteringOptimized, self).__init__()
    ...
    def forward(self, x, labels=None):
        ...
        # 事前にサイズを確保してリストの再割り当てを避ける（高速化）
        batch_size = x.shape[0]
        outputs = []
        # リストの容量を事前に確保（Pythonでは自動的に最適化される）
        
        for i in range(self.n_kernels_):
            sub_net = self.sub_nets[i]
            embeddings = sub_net.encoder(x)
            output_probs = sub_net(embeddings)
            outputs.append(output_probs.view(batch_size, 1, -1))

        # torch.cat()を最適化（事前にサイズが分かっている）
